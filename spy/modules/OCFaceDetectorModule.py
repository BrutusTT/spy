...
class OCFaceDetectorModule(BaseModule):
    """ The OCFaceDetectorModule class provides a yarp module for recognizing faces based on 
    ...
    def onImage(self, cv2_image):
        ...
        faces = OCFaceDetectorModule.HC['frontalface_default'].detectMultiScale(gray, 1.3, 5)
        
        for (x,y,w,h) in faces:
            cv2.rectangle(cv2_image,(x,y),(x+w,y+h),(255,0,0),2)
            roi_gray  = gray[y:y+h, x:x+w]
            roi_color = cv2_image[y:y+h, x:x+w]
            eyes = OCFaceDetectorModule.HC['eye'].detectMultiScale(roi_gray)
